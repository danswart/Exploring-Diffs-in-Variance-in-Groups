---
title: "Exploring Sample Variances for Distribution Types"
...
# Create a random universe (1000 random integers between 1 and 300)
random_universe <- base::sample(1:300, 1000, replace = TRUE)

# Create non-random universes (several options)

# Option 1: Sequential numbers
sequential_universe <- base::seq(1, 300, length.out = 1000)

# Option 2: Clustered data (4 distinct clusters)
clustered_universe <- c(
  stats::rnorm(250, mean = 30, sd = 6),
  stats::rnorm(250, mean = 100, sd = 6),
  stats::rnorm(250, mean = 180, sd = 6),
  stats::rnorm(250, mean = 270, sd = 6)
)
# Round to integers and ensure values are within 1-300
clustered_universe <- base::pmin(300, base::pmax(1, base::round(clustered_universe)))

# Option 3: Bimodal distribution
bimodal_universe <- c(
  stats::rnorm(500, mean = 75, sd = 30),
  stats::rnorm(500, mean = 225, sd = 30)
)
# Round to integers and ensure values are within 1-300
bimodal_universe <- base::pmin(300, base::pmax(1, base::round(bimodal_universe)))

# Option 4: Increasing variance (first half has low variance, second half high)
increasing_var_universe <- c(
  stats::rnorm(500, mean = 150, sd = 15),
  stats::rnorm(500, mean = 150, sd = 60)
)
# Round to integers and ensure values are within 1-300
increasing_var_universe <- base::pmin(300, base::pmax(1, base::round(increasing_var_universe)))
...
ggplot2::ggplot(results_df, 
                ggplot2::aes(x = factor(sample_size),
                             y = variance, 
                             color = universe_type
                             )
                ) +
...
ggplot2::ggplot(results_df, ggplot2::aes(x = factor(sample_size), y = variance, color = universe_type)) +
  ggplot2::geom_boxplot() +
...
ggplot2::ggplot(results_summary, ggplot2::aes(x = factor(sample_size), y = mean_variance, color = universe_type)) +
...
for(size_name in names(results)) {
  ...
  diff_data <- base::rbind(diff_data, 
                           base::data.frame(
...
ggplot2::ggplot(diff_data, ggplot2::aes(x = factor(sample_size), y = variance_difference)) +
  ggplot2::geom_boxplot() +
...
mean_vars <- results_df |>
  dplyr::group_by(universe_type) |>
...
ggplot2::ggplot(results_df, 
  ...
  ggplot2::geom_vline(data = mean_vars,
                      ggplot2::aes(xintercept = mean_var, color = universe_type),
                      linetype = "dashed", linewidth = 1) +
  ggplot2::facet_wrap(~sample_size, 
                      scales = "free",
                      labeller = ggplot2::labeller(sample_size = ~ base::paste("Sample Size =", .x))) +
  ggplot2::labs(
    title = "Distribution of Sample Variances for 2 Universes by Sample Size",
  ...
  ggplot2::scale_fill_manual(
    name = "Universe Type",
    values = c("bi-modal" = "red", "random" = "blue"),
    labels = c("bi-modal" = "Bi-Modal", "random" = "Random")
  ) +
...
A bi-modal distribution isn't ***automatically*** high-variance. It depends on how far apart the modes are and how tightly clustered the values are around each mode. In this case, the relatively small sd=30 within each cluster keeps the bi-modal variance lower than the uniform.
...
for(size_name in names(results)) {
  ...
  # Store results
  diff_stats <- base::rbind(diff_stats, 
                      base::data.frame(
                        sample_size = size,
...
No — "better" isn't meaningful here. They're just different populations with different characteristics. The simulations demonstrate that:

* The sample variance is a consistent estimator (it gets more precise with larger samples)  
* Different population structures produce different variance magnitudes  
* You can't infer population shape from the distribution of sample variances alone   



#### The practical takeaway: 

If you're sampling from an unknown population, larger samples give you more reliable variance estimates regardless of the underlying distribution shape. But the variance estimate itself only tells you about spread—not whether the population is uniform, bi-modal, or something else entirely.
