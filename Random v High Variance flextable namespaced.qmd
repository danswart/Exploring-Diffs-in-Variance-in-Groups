...
subtitle: "Random vs Increasing Variance"
description: ""
authors: 
...
# Create bimodal distribution with extremely high variance
high_var_universe <- base::c(
  base::sample(1:100, 500, replace=TRUE),    # Cluster at the low end
  base::sample(901:1000, 500, replace=TRUE)  # Cluster at the high end
)
...
tibble::tibble(
  Universe = c("Random", "High Variance"),
  Variance = c(base::round(stats::var(random_universe), 2),
               base::round(stats::var(high_var_universe), 2)),
  `Expected Range` = c("~83,000 - 84,000", "Much higher")
) |>
  flextable::flextable() |>
  ...
  flextable::colformat_num(j = "Variance", big.mark = ",") |>
  flextable::theme_apa() |>  # THIS IS NICE
  # flextable::theme_booktabs() |>  # NICE, MORE COMPACT
  # flextable::theme_box() |>   # OK, INCLUDES CELL BORDERS
  flextable::bg(bg = "palegreen", part = "header") |> 
...
## Random Universe vs High Variance Universe
...
for(size in sample_sizes) {
  ...
  nonrandom_variances <- calculate_sample_variances(high_var_universe, size)
...
ggplot2::ggplot(results_df, 
       ggplot2::aes(x = factor(sample_size),
           y = variance, color = universe_type)
       ) +
  ggplot2::geom_boxplot() +
  ggplot2::labs(
    title = "Sample Variance by Sample Size and Universe Type",
    subtitle = "Random vs High Variance",
    x = "Sample Size",
    y = "Variance",
    color = "Universe Type"
  ) 
...
ggplot2::ggplot(diff_data, 
                ggplot2::aes(x = factor(sample_size), y = variance_difference)) +
  ggplot2::geom_boxplot() +
...
ggplot2::ggplot(results_df,
  ...
  ggplot2::geom_vline(data = results_df |> 
  ...
  ggplot2::labs(title = "Distribution of Sample Variances by Universe Type and Sample Size",
...
for(size_name in names(results)) {
  size <- base::as.numeric(gsub("size_", "", size_name))
...
ggplot2::ggplot(diff_stats, 
                ggplot2::aes(x = factor(sample_size), y = mean_diff)) +
  ggplot2::geom_point(size = 3) +
  ggplot2::geom_errorbar(ggplot2::aes(ymin = lower_ci, ymax = upper_ci), width = 0.2) +
  ggplot2::geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  ggplot2::labs(
    title = "Mean Difference in Sample Variances Between Random and Non-random Universes",
