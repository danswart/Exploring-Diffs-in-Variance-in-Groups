---
title: "Exploring Sample Variances for Different Universes"
subtitle: "Random v Clustered Universes"
description: ""
authors: 
...
# Random Universe v Clustered Universe
...
for(size in sample_sizes) {
  ...
  nonrandom_variances <- calculate_sample_variances(clustered_universe, size)
...
for(size_name in names(results)) {
  ...
  # Store results
  result_stats <- base::rbind(result_stats, 
                              base::data.frame(
    sample_size = size,
...
```{r}
ggplot2::ggplot(results_df, ggplot2::aes(x = variance, fill = universe_type)) +
  ggplot2::geom_density(alpha = 0.5) +
  ggplot2::geom_vline(data = results_df |> dplyr::group_by(universe_type) |> 
               dplyr::summarize(mean_var = mean(variance)),
...
ggplot2::ggplot(diff_stats, ggplot2::aes(x = factor(sample_size), y = mean_diff)) +
  ggplot2::geom_point(size = 3) +
