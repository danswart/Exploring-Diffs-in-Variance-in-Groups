...
autocorr_universe <- numeric(n_months)
autocorr_universe[1] <- district_baseline + rnorm(1, 0, 8)

for(i in 2:n_months) {
  # Each month's score depends on last month's + some random variation
  autocorr_universe[i] <- 0.1 * autocorr_universe[i-1] + 0.7 * district_baseline + rnorm(1, 0, 8)
}


# Keep scores realistic (0-100 range)
autocorr_universe <- pmax(0, pmin(100, autocorr_universe))
